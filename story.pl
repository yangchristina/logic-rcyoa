...
choice(scene(), num(1), "Swamp (dark and stormy)", ["1"], 0),
choice(scene(), num(2), "Magical candy castle in the clouds (light and magical)", ["1"], 0),
choice(scene(), num(3), "Desert (dry and windy and sunny)", ["1"], 0),
choice(scene(), num(4), "Virtual world (futuristic, EDM music)", ["1"], 0)
]).
...
choice(scene(1), num(1), "Throw away the crown. What rubbish!", [scene(2), scene(11)], 1), % num may be switched to just an int? not sure rn
choice(scene(1), num(2), "Love frogs, so you agree to save them, but secretly collect information about them so you can betray the butterflies later", [scene(4)], -1).
choice(scene(1), num(3),"Point your sword to the sky, and declare you will save the butterflies, and destroy the frogs!", [scene(4), scene(10), scene(3), scene(5)], 9).
choice(scene(1), num(4), "As a seasoned ruler, you calmly order the butterflies to bring you a plate of jelly beans.", [scene(2)], 7).
...
choice(scene(2), num(1), "'Of course! They will pay for this great humiliation!'", [scene(4), scene(3)], 7).
choice(scene(2), num(2), "'Destroy them? Never! They've already destroyed me!'", [scene(11), scene(9), scene(5)], 2).
choice(scene(2), num(3), "'Will I be paid? Is this job? My greatest strength is that I will sell my soul to be employed.'", [scene(3), scene(4), scene(5)], 5).
choice(scene(2), num(4), "'DEATH TO THE FROGS. DEATH TO THE FROGS. DEATH TO THE FROGS. DEATH TO THE FROGS. DEATH TO THE FROGS. DEATH TO THE FROGS. DEATH TO THE FROGS. DEATH TO THE FROGS. DEATH TO THE FROGS.'", [scene(3), scene(5)], 9).
...
choice(scene(3), num(1), "Turn back, nope, this dark forest is not for meeeee", [scene(1), scene(9), scene(11)], 2).
choice(scene(3), num(2), "Send butterfly scouts deep into the woods, but it does not seem likely that they will return", [scene(10.5)], -3).
choice(scene(3), num(3), "Bravely forge ahead.", [scene(5)], 8).
choice(scene(3), num(4), "The frogs will protect me I will be safe.", [scene(5), scene(2)], 1).
choice(scene(3), num(5), "Scout around the area for tools that may be useful.", [scene(4), scene(5)], 7).
...
choice(scene(4), num(1), "Wow, this lamp seems useful", [scene(3)], 8).
choice(scene(4), num(2), "I need nothing but my own courage and skills", [scene(5)], 6).
choice(scene(4), num(3), "Give me everything you offer", [scene(5)], 4).
choice(scene(4), num(4), "Money, money, money, MONEYYYYYYY!!!!!", [scene(5)], 10).

scene_desc(scene(5), "You've run into a giant frog henchman. What will you do next?").
choice(scene(5), num(1), "Flee. My life is not worth this.", [scene(11), scene(10.5), scene(9)], 1).
choice(scene(5), num(2), "Sacrifice the butterflies. Hey, I've got more important things to do", [scene(10.5), scene(10), scene(6), scene(8)], -5).
choice(scene(5), num(3), "Prepare for battle. Even though I've spent everyday job searching... Wish me luck.", [scene(6), scene(10), scene(7)], 8).
choice(scene(5), num(4), "Prepare for battle! I've got this!", [scene(8), scene(6), scene(7)], 8).
choice(scene(5), num(5), "I can convince this frog that I'm on his side.", [scene(7), scene(6), scene(8)], 6).
...
choice(scene(6), num(1), "No matter, I am prepared to move forward", [scene(10), scene(5), scene(10.5)], 8).
choice(scene(6), num(2), "Oh no, I've got to turn back and recoup", [scene(9), scene(11)], 4).
choice(scene(6), num(3), "Speed up to apprehend the henchman again", [scene(3), scene(5), scene(10), scene(10.5)], 7).
choice(scene(6), num(4), "I'm on the frogs' side? Why should I stop it. Pretend to accidentally let it go.", [scene(10), scene(10.5)],-1).
...
choice(scene(7), num(1), "Dispose of it. That way, no one will be able to alert the frogs. And let's continue onwards", [scene(10)], 10).
choice(scene(7), num(2), "How could I forget? I am with the frogs. Let it go, but its defeat shall lure the butterflies into a false sense of security, Mwahhahahahahahah.", [scene(3), scene(2), scene(10.5), scene(10)], -2).
choice(scene(7), num(3), "I'll show it some mercy but just to be safe, I'll send it back to the butterflies.", [scene(9), scene(10)], 7).
choice(scene(7), num(4), "I'll show some mercy, it will guide me to the frog monarch or else!", [scene(10)], 8).
...
choice(scene(8), num(1), "Run away! The frogs are too scary!", [scene(9), scene(11)], 1).
choice(scene(8), num(2), "I was just pretending to be defeated, I'm  on the frogs' side, remember? This is also part of my plan", [scene(10.5)], -2).
choice(scene(8), num(3), "Evade capture! The frogs may have won this battle, but I'll win the war!", [scene(10)], 6).
choice(scene(8), num(4), "I'll allow myself to be captured, but that just means I'll be brought right to the frog monarch, where I'll show it my might!", [scene(10.5)], 8).
...
choice(scene(9), num(1), "I'm going back home, it's not worth it! Send me home!", [scene(11), end()], 1).
choice(scene(9), num(2), "I can keep going, let me try again.", [scene(3), scene(5), scene(10)], 7).
choice(scene(9), num(3), "I guess I can keep going? Give me more money and I'll do it.", [scene(4)], 8).
...
choice(scene(10), num(1), "Fight! I will never give up", [end()], 7).
choice(scene(10), num(2), "Surrender, frogs are too scary. I'm sorry, butterflies, but I can't!", [end()], -10).
choice(scene(10), num(3), "Is it too late to run away?", [end()], 2).
choice(scene(10), num(4), "I've prepared this long, I guess I'll do my best.", [end()], 7).
...
choice(scene(10.5), num(1), "Fight! I will never give up", [end()], 7).
choice(scene(10.5), num(2), "Surrender, frogs are too scary. I'm sorry, butterflies, but I can't!", [end()], -100).
choice(scene(10.5), num(3), "Is it too late to run away?", [end()], (-2)).
choice(scene(10.5), num(4), "I've prepared this long, I guess I'll do my best.", [end()], 7).
...
choice(scene(11), num(1), "Stay", [end(*)], 1).                 % game will end
choice(scene(11), num(2), "Continue the adventure", [scene(3)], 1).  % game continues
choice(scene(11), num(3), "Go home", [end()], 1).                % game will end
